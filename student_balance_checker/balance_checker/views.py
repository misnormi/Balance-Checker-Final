...
def import_data(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            optional_columns = ['rfid_number', 'course', 'balance', 'date']
            ...
            for row_num, row in enumerate(reader, start=2):  # Start at 2 because of header
                try:
                    ...
                    # Handle invalid student_id - generate one if needed
                    original_student_id = student_id
                    if not student_id or student_id.upper() in ['#N/A', 'N/A', 'NULL', '']:
                        # Generate a student ID based on row number and name
                        name_part = fullname.replace(' ', '').replace('#', '').replace('/', '').replace('\\', '')[:5].upper()
                        if not name_part:
                            name_part = "STU"
                        student_id = f"{name_part}{row_num:04d}"
                        # Log the generated student ID
                        errors.append(f'Row {row_num}: Generated student ID "{student_id}" for invalid ID "{original_student_id}"')
                    
                    # Validate fullname - skip only if completely empty
                    if not fullname or fullname.upper() in ['#N/A', 'N/A', 'NULL', '']:
                        # Generate a name if completely invalid
                        fullname = f"Student {row_num}"
                    ...
                    # Get optional fields
                    rfid_raw = row.get('rfid_number', '').strip()
                    course = row.get('course', '').strip() or None
                    balance = row.get('balance', '0').strip()
                    date_str = row.get('date', '').strip()
                    
                    # Handle RFID number - be more permissive
                    rfid_number = None
                    if rfid_raw and rfid_raw.upper() not in ['#N/A', 'N/A', 'NULL', '0', '']:
                        rfid_number = rfid_raw
                    elif rfid_raw and rfid_raw.upper() in ['#N/A', 'N/A', 'NULL']:
                        # For #N/A RFID, we'll leave it as None (no RFID)
                        rfid_number = None
                    ...
                    if date_str:
                        try:
                            ...
                            if parsed_date is None:
                                errors.append(f'Row {row_num}: Invalid date format "{date_str}". Use YYYY-MM-DD or MM/DD/YYYY format.')
                                continue
                    ...
                    # Check for RFID number conflicts before creating/updating
                    if rfid_number:
                        existing_rfid_student = Student.objects.filter(rfid_number=rfid_number).exclude(student_id=student_id).first()
                        if existing_rfid_student:
                            errors.append(f'Row {row_num}: RFID number "{rfid_number}" is already used by student {existing_rfid_student.student_id}')
                            continue
                    ...
                    try:
                        ...
                        # Add date to balance_updated_at if provided
                        if parsed_date:
                            student_data['balance_updated_at'] = timezone.make_aware(parsed_date)
                        ...
                        if created:
                            ...
                            if rfid_number:
                                # Double-check RFID is not already used by another student
                                existing_rfid_student = Student.objects.filter(rfid_number=rfid_number).exclude(student_id=student_id).first()
                                if existing_rfid_student:
                                    errors.append(f'Row {row_num}: RFID number "{rfid_number}" is already used by student {existing_rfid_student.student_id}')
                                    continue
                            ...
                            # Update balance_updated_at if date provided
                            if parsed_date:
                                student.balance_updated_at = timezone.make_aware(parsed_date)
                        ...
                        if 'UNIQUE constraint failed' in str(db_error):
                            if 'rfid_number' in str(db_error):
                                errors.append(f'Row {row_num}: RFID number "{rfid_number}" is already in use by another student')
                            else:
                                errors.append(f'Row {row_num}: Database constraint error - {str(db_error)}')
                        else:
                            errors.append(f'Row {row_num}: Database error - {str(db_error)}')
                        continue
